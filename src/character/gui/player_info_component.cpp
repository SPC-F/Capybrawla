...
namespace PlayerInfoComponent {
  ...
  GameObject& left_col(Scene &scene, PlayerObject &player) {
    ...
    auto &player_skin_image = scene.add_game_object<UIImage>(scene, "", left_col_width, left_col_height, Point{}, Point{});
  ...
  GameObject& right_col(Scene &scene, const PlayerObject &player) {
    ...
    if (!player_controller_opt.has_value()) {
      throw std::runtime_error("A player must have a PlayerController behavior registered in order to create a player_info gui element");
    }

...
void WeaponBatInteractableBehavior::on_start() {
    ...
    collider_opt->get().add_on_trigger_enter([this](Collider2D& self, Collider2D& other) {
        ...
        auto* bat_behavior = dynamic_cast<WeaponBatInteractableBehavior*>(&behavior);
        if (!bat_behavior) return;

        bat_behavior->pickup_weapon(self, other);
    });
...
void WeaponBatInteractableBehavior::pickup_weapon(Collider2D& self, Collider2D& other) {
    ...
    while (current_opt.has_value()) {
        ...
        if (obj.tag() == "Player") {
            ...
            controller_opt->get().switch_weapon<WeaponBatPlayerObject>();
...
void WeaponBatInteractableBehavior::on_update(float dt) {
